// Package httpclient provides convenience and sensible defaults for some low level httpclient client
package httpclient
...
const tracerName = "httpclient-client"
...
// HTTPRequest represents a httpclient request
...
func (c Client) handle(
	...
	log.Ctx(ctx).Debug().Func(ops.LogHTTPRequest(req)).Msg("httpclient client request")
	...
	if err != nil {
		log.Ctx(ctx).Error().Func(ops.LogHTTPResponse(req, resp, err)).Msg("httpclient client response")
	} else {
		log.Ctx(ctx).Debug().Func(ops.LogHTTPResponse(req, resp, err)).Msg("httpclient client response")
	}
